...
class HypoLabPipeline:
    ...
    def run(self, skip_literature: bool = False) -> Dict[str, Any]:
        ...
        self._log(f"   Completed: {summary['completed']}, Significant: {summary['significant']}")
    ...
    def get_report(self) -> Dict[str, Any]:
        ...
        return {
            "profile_summary": self.profile.get("schema_summary", "") if self.profile else "",
    ...
    def to_markdown(self) -> str:
        ...
        for i, (hyp, res) in enumerate(zip(report["hypotheses"], report["test_results"]), 1):
            lines.append(f"### {i}. {hyp.get('test_type', 'Unknown').replace('_', ' ').title()}\n")
            ...
            if res.get("status") == "completed":
                ...
                for key in ["pearson_r", "f_statistic", "chi2", "t_statistic", "r_squared"]:
                    if key in res:
                        lines.append(f"- **{key.replace('_', ' ').title()}:** {res[key]}\n")
            else:
                lines.append(f"**Result:** {res.get('status', 'Unknown')} — {res.get('reason', '')}\n")
        ...
        lines.append(f"- Skipped: {summary.get('skipped', 0)} | Errors: {summary.get('errors', 0)}\n")

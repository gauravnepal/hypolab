...
def generate_ground_truth(
    n: int = 1000, noise: float = 0.3, seed: int = 42
) -> pd.DataFrame:
    """
...
def test_pipeline_finds_real_relationships(ground_truth_df):
    ...
    assert summary["significant"] >= 3, (
        f"Expected >=3 significant, got {summary['significant']}"
    )
...
def test_pipeline_rejects_noise_hypotheses(ground_truth_df):
    ...
    for hyp, res in noise_hyps:
        is_rejected = res.get("status") == "skipped" or not res.get(
            "significant", False
        )
        assert is_rejected, (
    ...
    print(
        f"Real signals found: {summary['significant']}/{summary['completed']} significant."
    )
